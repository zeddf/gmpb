...
function SWEP:PrimaryAttack(data)
	...
	self.Weapon:SetNextPrimaryFire( CurTime() + 0.115 )
	...
	if pb:IsValid() then
		...
		local acuracy = 100 // The higher the number the more accurate, the lower the number it is more accurate. The number should never be below 40 or 50... unless you want a super inacurate gun..
		...
		if phy:IsValid() then phy:ApplyForceCenter(pb:GetForward()*500000)
		end
    	end
end 
...
/*function SWEP:SecondaryAttack(data)
	...
	//self.BaseClass.ShootEffects (self)
	self.Weapon:EmitSound(Sound( "marker/pbfire.wav" ))
	self.Weapon:SetNextSecondaryFire( CurTime() + 0.115 )
	self:TakePrimaryAmmo(1)
	if ( !self:CanPrimaryAttack() ) then return end    
	...
	if pb:IsValid() then
		...
		local b = ents.Create( "info_target" )
		if (ValidEntity(b)) then
			local acuracy = 5 // This number is the accuracy... Higher number = less accurate, lower numbar = more accurate
			b:SetPos( self.Owner:GetEyeTrace( ).HitPos + Vector(math.random(-acuracy,acuracy),math.random(-acuracy,acuracy),math.random(-acuracy,acuracy)))
			b:Spawn( )
			pb:PointAtEntity(b)
			b:Remove( )
		end
