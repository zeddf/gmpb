...
function GM:HUDDrawScoreBoard()
	...
	surface.SetTextPos( xOffset + boardWidth - (colWidth*2) + 8,	y)	surface.DrawText("Outs")
	...
	for team,info in pairs(ScoreboardInfo) do
		...
		local teamText = info.TeamName .. (" | Wins: ") .. tostring(GetGlobalInt(info.TeamName .. "Wins")) .. (" | Score: ") .. tostring(GetGlobalInt(info.TeamName .. "Score")) .. " | Players: " .. #info.Players
		...
		for index,plinfo in pairs(info.Players) do
			...
			if (plinfo.PlayerObj:IsOut()) then
			...
			elseif (plinfo.PlayerObj == LocalPlayer()) then
			...
			if plinfo.PlayerObj:IsOut() then textcolor = Color( 60, 60, 60, alpha ) end
